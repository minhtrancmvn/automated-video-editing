...
def test_detect_capabilities_parses_versions_architecture_memory_and_encoders(
    monkeypatch,
) -> None:
    ...
    def run(args, **kwargs):
        ...
        if args[0] == "ffmpeg" and "-encoders" not in args:
            return subprocess.CompletedProcess(
                args, 0, "ffmpeg version 7.0.1 Copyright", ""
            )
        if args[0] == "ffprobe":
            return subprocess.CompletedProcess(
                args, 0, "ffprobe version 7.0.1 Copyright", ""
            )
        return subprocess.CompletedProcess(
            args,
            0,
